...
public class comment {
	 ...
	 public static void main(String[] args) throws IOException {
			...
			while((weibo_temp = weibo.readLine()) != null) {
				...
				else if( weibo_data[4].equals("3") || weibo_data[4].equals("4")) {
					counter_4+=1;
					for(int i = 0; i < tfidf_data.length; i++) {
				...
				else if(Integer.parseInt(weibo_data[4])>4 &&  Integer.parseInt(weibo_data[4])<9) {
					counter_5+=1;
					for(int i = 0; i < tfidf_data.length; i++) {
				...
				else if(Integer.parseInt(weibo_data[4])>8) {
					counter_6+=1;
					for(int i = 0; i < tfidf_data.length; i++) {
			...
			weibo.close();
		 	...
		 	BufferedReader fp = new BufferedReader(new InputStreamReader(new FileInputStream("D:/New/AliTianChi-master/AliTianChi-master/新浪微博互动预测大赛/Season1/trick/weibo_pred.txt"), "UTF-8"));
		 	BufferedWriter wp = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("D:/New/AliTianChi-master/AliTianChi-master/新浪微博互动预测大赛/Season1/trick/weibo_pred_c.txt"), "UTF-8"));
		 	String final_temp;
		 	String fp_temp;
		 	int cc = 0;
		 	...
		 	while((final_temp = tfidf_2.readLine()) != null) {
		 		...
		 		if(Max == sum_1) {
		 			...
		 			wp.write(fp_data[0]+"\t"+fp_data[1]+"\t"+fp_data[2]+","+6);
		 			wp.write("\r\n");
	 			}
		 		
	 			else if(Max == sum_6) {
		 			System.out.println(cc+" "+"sum_6"+" "+Max);
		 			wp.write(fp_data[0]+"\t"+fp_data[1]+"\t"+fp_data[2]+","+481);
		 			wp.write("\r\n");
	 			}
