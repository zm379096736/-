...
public class proj_final {
	 public static void main(String[] args) throws IOException {
			...
			while((weibo_temp = weibo.readLine()) != null) {
				...
				else if(weibo_data[3].equals("2") || weibo_data[3].equals("3")) {
					counter_3+=1;
					for(int i = 0; i < tfidf_data.length; i++) {
				...
				else if( Integer.parseInt(weibo_data[3])>3 &&  Integer.parseInt(weibo_data[3])<8) {
					counter_4+=1;
					for(int i = 0; i < tfidf_data.length; i++) {
				...
				else if(Integer.parseInt(weibo_data[3])>7 &&  Integer.parseInt(weibo_data[3])<22) {
					counter_5+=1;
					for(int i = 0; i < tfidf_data.length; i++) {
				...
				else if(Integer.parseInt(weibo_data[3])>21) {
					counter_6+=1;
					for(int i = 0; i < tfidf_data.length; i++) {
			...
			weibo.close();
		 	...
		 	BufferedReader fp = new BufferedReader(new InputStreamReader(new FileInputStream("D:/New/AliTianChi-master/AliTianChi-master/新浪微博互动预测大赛/Season1/trick/weibo_predict_data.txt"), "UTF-8"));
		 	BufferedWriter wp = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("D:/New/AliTianChi-master/AliTianChi-master/新浪微博互动预测大赛/Season1/trick/weibo_pred.txt"), "UTF-8"));
		 	String final_temp;
		 	String fp_temp;
		 	int cc = 0;
		 	...
		 	while((final_temp = tfidf_2.readLine()) != null) {
		 		...
		 		if(Max == sum_1) {
		 			...
		 			wp.write(fp_data[0]+"\t"+fp_data[1]+"\t"+0);
		 			wp.write("\r\n");
		 		}
		 		
		 		else if(Max == sum_2) {
		 			System.out.println(cc+" "+"sum_2"+" "+Max);
		 			wp.write(fp_data[0]+"\t"+fp_data[1]+"\t"+1);
		 			wp.write("\r\n");
		 		}
		 		
		 		else if(Max == sum_3) {
		 			System.out.println(cc+" "+"sum_3"+" "+Max);
		 			wp.write(fp_data[0]+"\t"+fp_data[1]+"\t"+2);
		 			wp.write("\r\n");
	 			}
	 			
	 			else if(Max == sum_4) {
		 			System.out.println(cc+" "+"sum_4"+" "+Max);
		 			wp.write(fp_data[0]+"\t"+fp_data[1]+"\t"+5);
		 			wp.write("\r\n");
				}
	 			
				else if(Max == sum_5) {
		 			System.out.println(cc+" "+"sum_5"+" "+Max);
		 			wp.write(fp_data[0]+"\t"+fp_data[1]+"\t"+14);
		 			wp.write("\r\n");
	 			}
		 		
	 			else if(Max == sum_6) {
		 			System.out.println(cc+" "+"sum_6"+" "+Max);
		 			wp.write(fp_data[0]+"\t"+fp_data[1]+"\t"+1297);
		 			wp.write("\r\n");
	 			}
